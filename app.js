...
function App() {
  ...
  async function analyze() {
    ...
    // Set rate limit timer if using default/shared key
    if (isUsingDefaultKey && processingMode === "online") {
...
${resumeText.slice(0, 4000)}`;
        const rawDomain = processingMode === "online" 
          ? await callGroq(domainPrompt, groqKey)
...
JSON only:`;
      ...
      const rawExtract = processingMode === "online" 
          ? await callGroq(extractPrompt, groqKey)
...
- **Experience**: Score based on relevance and impact. Unrelated jobs = low score. Relevant roles with achievements = high score.
- **Projects**: Score based on technical depth and relevance. "To-do app" = low. Production-level or innovative projects = high.
- **Education**: Score based on relevance to role. Any degree gets minimum 50. Relevant degree = 70-80. Irrelevant degree = 50-60.
- **Internship**: Score based on relevance and recency. Unrelated internships = 40-50. Relevant ones = 70-80.
- **Certificate**: Score based on relevance and credibility. Generic online courses = 40-50. Industry certifications = 70-90.
- **Extracurricular**: Score based on leadership/relevance. Generic membership = 30-40. Leadership or relevant activities = 60-80.

Return ONLY valid JSON with ATS scores. No markdown, no extra text.

{
  ...
  "section_scores": {
    "skills": { "score": 80, "feedback": "Specific honest feedback about what's good or missing" },
    "experience": { "score": 70, "feedback": "Honest assessment of experience relevance and depth" },
    "education": { "score": 75, "feedback": "How well education aligns with target role" },
    "projects": { "score": 60, "feedback": "Technical depth and relevance to role - be critical" },
    "internship": { "score": 85, "feedback": "Relevance and quality of internships" },
    "certificate": { "score": 50, "feedback": "Value and relevance of certifications" },
    "extracurricular": { "score": 40, "feedback": "Leadership and relevance to career goals" }
  },
  "strengths": ["specific strength 1", "specific strength 2", "specific strength 3"],
  "critical_issues": ["specific issue 1", "specific issue 2", "specific issue 3"],
  "verdict": "2 sentence honest professional assessment - don't sugarcoat."
}

Attempt to score ALL 7 sections listed in section_scores.

### ROBUST PROFILE DETECTION:
Look at the Header area. If a platform name (LinkedIn, GitHub, Hackerrank, Kaggle) or a handle is physically present in the text, mark it as present: true and score 100. DO NOT ignore platforms that are mentioned by name.
...
JSON only:`;
      ...
      const rawScores = processingMode === "online" 
          ? await callGroq(scorePrompt, groqKey)
...
${JSON.stringify(parsed.sections, null, 2).slice(0, 3000)}`;
      ...
      if (processingMode === "online") {
        await streamGroq(reviewPrompt, (text) => setReviewText(text), groqKey);
      } else {
...
JSON array only:`;
      ...
      try {
        const adjacentRaw = processingMode === "online"
          ? await callGroq(adjacentPrompt, groqKey)
            ...
            <div className="card">
              ...
              {processingMode === "online" ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <div className="model-row">
                    <div className="ollama-pill" style={{ borderColor: groqKey ? 'var(--primary)' : 'var(--danger)', background: groqKey ? 'var(--primary-dim)' : 'var(--danger-dim)', color: groqKey ? 'var(--primary)' : 'var(--danger)' }}>
                      <div className={`dot ${groqKey ? 'dot-green' : 'dot-red'}`}/> 
                      {groqKey ? "Engine Ready" : "Enter API Key"}
                    </div>
              ...
              <button className="btn-analyze" style={{marginTop:"1rem"}}
                ...
                disabled={(processingMode === "offline" && (!ollamaOk || !model || checking)) || 
                         (processingMode === "online" && (!groqKey || (isUsingDefaultKey && waitTime > 0)))}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                {(isUsingDefaultKey && waitTime > 0 && processingMode === "online") 
              ...
              {processingMode === "online" && (
                <div style={{
                  marginTop: "1rem",
                  padding: "12px",
                  background: isUsingDefaultKey ? "rgba(245,158,11,0.1)" : "rgba(16,185,129,0.1)",
                  border: `1px solid ${isUsingDefaultKey ? "rgba(245,158,11,0.3)" : "rgba(16,185,129,0.3)"}`,
                  ...
                  {isUsingDefaultKey ? (
                    <>
                      ...
                      <div style={{marginBottom: "6px"}}>
                        You're using a shared demo API key. To avoid rate limits, you must wait <strong>{RATE_LIMIT_SECONDS} seconds</strong> between analyses.
                      </div>
                      <div>
                        💡 <strong>Get your own free key</strong> at{" "}
                    ...
                    <>
                      ...
                      <div>
                        You can analyze resumes without the {RATE_LIMIT_SECONDS}-second timer. Enjoy unlimited analyses!
                      </div>
                    </>
                  )}
                </div>
